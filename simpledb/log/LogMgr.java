...
public class LogMgr implements Iterable<BasicLogRecord> {
	...
	/**
	 * Returns an iterator for the log records, which will be returned in
	 * forward order starting from the last checkpoint.
	 */
	public synchronized Iterator<BasicLogRecord> iteratorForward() {
		flush();
		return new LogForwardIterator(currentblk);
	}
	
	/**
	 * Returns an iterator for the log records, which will be returned in
	 * forward order starting with the oldest.
	 */
	public synchronized Iterator<BasicLogRecord> iteratorForwardFromStart() {
		flush();
		return new LogForwardFromStartIterator(currentblk);
	}
