...
public class RecoveryMgr {
	...
	public int setInt(Buffer buff, int offset, int newval) {
		...
		if (isTempBlock(blk))
			...
			return new SetIntRecord(txnum, blk, offset, newval, oldval).writeToLog();
		}
	}
	...
	public int setString(Buffer buff, int offset, String newval) {
		...
		if (isTempBlock(blk))
			...
			return new SetStringRecord(txnum, blk, offset, newval, oldval).writeToLog();
	}
	...
	public void doRecover() {
		LinkedList<Integer> finishedTxs = new LinkedList<Integer>();
		...
		while (iter.hasNextFwd()) {
			LogRecord rec = iter.nextFwd();
			if (rec != null && finishedTxs.contains(rec.txNumber())) {
